...
const ViewDispersal = ({ open, onClose,data}) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      ...
      <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-md">
          ...
          <Field label="Raiser Name" value={data.raiserName} />
          <Field label="Quantity" value={data.quantity} />
          <Field label="Status" value={data.status } />
          <Field
            label="Dispersal Date"
            value={
              data.dispersalDate
                ? new Date(data.dispersalDate).toLocaleDateString()
                : "-"
            }
          />
